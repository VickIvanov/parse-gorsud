...
class LegalAssistant:
    ...
    def delete_all_cloud_files(self):
        """Удаляет все файлы из облака через SDK, как в примере из документации Яндекса"""
        try:
            ...
            for file in files:
                print(f"\n⏳ Удаляем файл: {getattr(file, 'name', file.id)} (ID: {file.id})")
                try:
                    file.delete()
                    deleted_count += 1
                    print("✅ Успешно")
                except Exception as e:
                    print(f"❌ Ошибка удаления файла {file.id}: {str(e)}")
